...
def generate_train_dev_dataset_from2files(train_filepath, sent_vocab, tag_vocab):
    """ Read corpus from given train file path and the dev file path which is got by modification on train_filepath
    Args:
        train_filepath: file path
        sent_vocab: sentence vocab
        tag_vocab: tag vocab
        train_proportion: proportion of training data
    Returns:
        ...
        dev_data: data for development, list of tuples, each containing a sentence and corresponding tag.
    """
    sentences, tags = read_corpus(train_filepath)
    sentences = words2indices(sentences, sent_vocab)
    tags = words2indices(tags, tag_vocab)
    train_data = list(zip(sentences, tags))

    dev_filepath = train_filepath.replace('train.txt', 'dev.txt')
    sentences, tags = read_corpus(dev_filepath)
    sentences = words2indices(sentences, sent_vocab)
    tags = words2indices(tags, tag_vocab)
    dev_data = list(zip(sentences, tags))
    return train_data, dev_data
